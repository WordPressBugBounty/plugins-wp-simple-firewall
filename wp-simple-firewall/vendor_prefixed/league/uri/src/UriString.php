...
namespace AptowebDeps\League\Uri;

use AptowebDeps\League\Uri\Exceptions\IdnaConversionFailed;
use AptowebDeps\League\Uri\Exceptions\IdnSupportMissing;
use AptowebDeps\League\Uri\Exceptions\SyntaxError;
use AptowebDeps\League\Uri\Idna\Idna;
use TypeError;
...
final class UriString
{
    ...
    private static function isIpHost(string $ip_host): bool
    {
        ...
        $pos = strpos($ip_host, '%');
        if (false === $pos || 1 === preg_match(self::REGEXP_INVALID_HOST_CHARS, rawurldecode(substr($ip_host, $pos)))) {
            return false;
        }
        $ip_host = substr($ip_host, 0, $pos);
        return false !== filter_var($ip_host, FILTER_VALIDATE_IP, FILTER_FLAG_IPV6) && 0 === strpos((string) inet_pton($ip_host), self::ZONE_ID_ADDRESS_BLOCK);
    }
}