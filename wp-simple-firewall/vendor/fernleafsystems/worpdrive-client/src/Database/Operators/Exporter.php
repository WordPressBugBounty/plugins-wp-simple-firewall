...
use FernleafSystems\WorpdriveClient\Database\Operators\Table\{
	TableHelper,
	TableRowsSqlBuilder
};
...
class Exporter {
	...
	public function buildTableDataStructureRows( string $table ) :void {
		...
		// Select all the Rows
		$rows = WorpdriveRuntime::host()->database()->selectCustom( sprintf(
			'SELECT %s FROM %s %s;',
			$tableHelper->insertableColumnSelectList(),
			$idEscaper->escape( $table ),
			$this->cfg->has( 'where' ) ? ' WHERE '.$this->cfg->get( 'where' ) : ''
		) );
		if ( !\is_array( $rows ) ) {
			throw new \Exception( \sprintf( 'Database query failed for table: %s', $table ) );
		}

		$this->totalDataRows = ( $this->totalDataRows ?? 0 ) + \count( $rows );
		if ( empty( $rows ) ) {
			return;
		}

		if ( $this->cfg->has( 'single-transaction' ) ) {
			if ( WorpdriveRuntime::host()->database()->doSql( "SET GLOBAL TRANSACTION ISOLATION LEVEL REPEATABLE READ; START TRANSACTION;" ) === false ) {
		...
		if ( $this->cfg->has( 'single-transaction' ) ) {
			if ( WorpdriveRuntime::host()->database()->doSql( 'COMMIT;' ) === false ) {
